...
const ConversationName: FunctionComponent<IConversationname> = () => {
  ...
  const rawAvatar = participants.filter(i => i.isActive === true).map((i) => i.avatar);
  ...
  return (
    <div className="flex gap-4 h-16 items-center">
      ...
      <div>
        <Avatar
          avatar={rawAvatar.length > 0 ? rawAvatar : newAvatar.filter((i) => i.id !== id).filter(i => i.isActive === true).map((i) => i.avatar)}
      ...
      <div className="flex flex-col items-start gap-2">
        ...
        {isGroup ? (
          <h4 className="text-sm text-color-base-100 font-medium">{`${participants
            .filter(i => i.isActive === true).length} members`}</h4>
        ) : status === 'none' ? (
