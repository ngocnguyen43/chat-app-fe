...
listenerMiddleware.startListening({
    ...
    effect: (action, listenerApi) => {
        ...
        const { information: { entity: { userId } }, contacts } = listenerApi.getState() as ApplicationState
        ...
        listenerApi.dispatch(setCurrentConversation({
            id: conversationId,
            participants: participants.filter(p => p.id !== userId),
            isGroup,
            state,
            name: participants.filter(p => p.id !== userId).map(p => p.fullName).join(" "),
