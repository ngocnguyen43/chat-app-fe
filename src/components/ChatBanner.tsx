...
const ChatBanner: FunctionComponent<IChatBanner> = (props) => {
  ...
  const { mutate: deleteRequest } = useDeleteFriendRequest(otherParticipant ? otherParticipant[0].id : undefined, data?.id)
          ...
          <div className="flex flex-col gap-4 items-center justify-center">
            ...
            <div className="w-full flex gap-2 items-center justify-center ">
              {(data && Object.keys(data).length && data.status !== "accepted" && data.requesterId === userId) ?
                <button className='text-color-base-100 font-medium bg-surface-mix-300 w-[150px] transition-all py-2 rounded-lg hover:scale-105' onClick={() => deleteRequest(undefined, { onSuccess: () => { refetch() } })}>Cancel request</button> : null}
              {(data && Object.keys(data).length && data.status !== "accepted" && data.recipientId === userId) ? <>
                <button className='text-color-base-100 font-medium bg-surface-mix-300 w-[150px] transition-all py-2 rounded-lg hover:scale-105' onClick={() => acceptRequest(undefined)}>Accept</button>
                <button className='text-color-base-100 font-medium bg-surface-mix-300 w-[150px] transition-all py-2 rounded-lg hover:scale-105' onClick={() => deleteRequest(undefined, { onSuccess: () => { refetch() } })}>Decline</button>
              </> : null}
              {(data && !Object.keys(data).length) ? <button className='text-color-base-100 font-medium bg-surface-mix-300 w-[150px] py-2 rounded-lg hover:scale-105' onClick={() => createFriendRequest(undefined, { onSuccess: () => { refetch() } })}>Send Request</button> : null}
            </div>
          </div>
        </div>
      ) : null}
