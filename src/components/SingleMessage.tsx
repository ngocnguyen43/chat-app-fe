...
const SingleMessage = React.forwardRef<MessageRef, ISingleMessage>((props, ref) => {
    ...
    return <>
        {<div ref={ref} className={clsx('w-full h-auto gap-4 flex py-2  relative transition-all origin-center', sender !== userId ? "flex-row" : "flex-row-reverse", isSelected ? "bg-purple-500/10 cursor-pointer" : "", message.length > 0 ? "px-72" : "px-64")} onClick={() => {
        ...
        >
            ...
            <div className={clsx('max-w-[500px] h-auto flex shrink-[1] flex-wrap relative ', sender === userId ? "cursor-pointer" : "")} onClick={() => {
                if (message.length === 0 && sender === userId && !isDelete) {
            ...
            {sender === userId && !isDelete && <div className={clsx("absolute  w-6 h-6 ring-4 ring-gray-500 rounded-full flex items-center justify-center bg-red-100 -translate-x-1/2 -translate-y-1/2 top-1/2 left-40 transition-all", message.length > 0 ? "opacity-100" : "opacity-0")}>
