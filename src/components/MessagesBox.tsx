...
const MessagesBox = () => {
    ...
    const content = data && data.pages.map(e => e.messages.map((c, i, arr) => {
        ...
        const shouldShowAvatar = i === arr.length - 1 || (i < arr.length - 2) && (c.sender !== arr[i + 1].sender || c.group !== arr[i + 1].group)
        return (
            <div key={Math.random() + c.createdAt}>
                {i < arr.length - 1 && c.group !== arr[i + 1].group && <div className='w-full flex items-center justify-center '>
                ...
                {i === arr.length - 1 && <div className='w-full lol flex items-center justify-center'>
                    <div className='bg-purple-400 my-3 p-2 rounded-2xl'>
                        <span className='text-white font-medium'>
                            {formatGroupedDate(c.group)}
                        </span>
    ...
    return (
        <div className='h-screen pb-12 w-full flex flex-col overflow-hidden px-[1px] transition-all' >
            ...
            {
                showTyping && <div className={clsx('w-full')}>
