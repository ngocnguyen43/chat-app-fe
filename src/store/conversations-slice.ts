...
const conversationsSlice = createSlice({
  ...
  reducers: {
    ...
    updateLastMessage: (state, action: PayloadAction<{ id: string, lastMessage: string, lastMessageAt: string, isLastMessageSeen: boolean, totalUnreadMessages: number }>) => {
    ...
    deleteConversations: (state, action: PayloadAction<string>) => {
      ...
      const present = [...state.entities]
      state.history = present
      state.entities = present.filter(item => item.conversationId !== id).sort((a, b) => {
        return +b.lastMessageAt - +a.lastMessageAt
      })
    },
...
export const { addConversations, deleteConversations, updateConversations, updateStatusConversation, rollbackConversations, updateLastMessage, updateLastDeletedMsg, updateTotalUnreadMessages } =
