...
export default function ConversationName() {
    ...
    return (
        <div className='flex gap-4 h-16 items-center'>
            ...
            <div>
                <Avatar avatar={avatar || savedAvatar} isGroup={isGroup || savedIsGroup} isOnline={isOnline || savedIsOnline} />
            </div>
            <div className='flex flex-col items-start'>
                <h2 className='text-xl font-semibold'>{name || savedName}</h2>
                {isGroup && <h4 className='text-sm'>10 members</h4>}
                {!(isGroup ?? savedIsGroup) ? ((isOnline ?? savedIsOnline) ? <h4 className='text-sm'>online</h4> : <h4 className='text-sm'>offline</h4>) : <></>}
            </div>
        </div>
    )
}
