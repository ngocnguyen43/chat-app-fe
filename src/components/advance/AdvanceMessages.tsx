...
export default function AdvanceMessages() {
    ...
    return (
        <>
            {type !== "none" && <div className='fixed w-full h-full backdrop-blur-sm top-0 left-0 flex items-center  justify-center z-30' onClick={handleOnClick}>
                <div ref={divRef}
                    ...
                    <div className="overflow-y-scroll flex shrink-[1] flex-wrap gap-2">
                        {
                            data && data.photos.length && data.photos.map((p) => {
                                return (
                                    <div
                                        key={p.id + id}
                                        className={clsx(
                                            'flex-1 rounded-[8px] overflow-hidden relative basis-[calc(50%-0.5rem)]',
                                        )}
                                    >
                                        <img
                                            src={p.url}
                                            alt=""
                                            className={clsx('w-full object-cover align-middle h-48')}
                                        />
                                    </div>
                                )
                            })
