    ...
    <div className="fixed w-full h-full top-0 left-0 backdrop-blur-sm z-10 flex items-center justify-center">
      <div className=" w-[500px] bg-white rounded-xl">
        ...
        <div className="flex items-center justify-end px-4 py-4 gap-4">
          <button className="text-white py-1 font-medium items-center flex  rounded-xl px-2 bg-gray-400"
            onClick={() => {
          ...
          <button disabled={otp.indexOf("") !== -1} onClick={() => {
            mutate(otp.join(""))
          }} className="text-white py-1 font-medium items-center flex hover:scale-105 transition-all rounded-xl px-2 bg-purple-500 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:scale-100">Active</button>
        </div>
      </div>
    </div>
  );
